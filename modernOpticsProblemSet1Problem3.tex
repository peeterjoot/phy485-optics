...
While we can still directly integrate the y-component equation once (as done above), our original assumed parameterization of \(\Br(s) = s(\cos\theta, \sin\theta)\) looses it's convenient form since we now have \(\theta = \theta(s)\) in the neighbourhood of the origin.  Once we choose to not neglect the step discontinuity, we have a coupled, much more difficult, system to deal with.
